...
//		}
		...
		if(mMenuViewSmall != null){
			if(mRoot.getChildCount() <= 0){
				if(mCallbacks!=null){
					mCallbacks.openMenuSetData(mContext, mMenuViewSmall);
				}
...
//				mRoot.addView(mMenuViewBig, 1, paramsBig);
			...
			throw new RuntimeException("the MenuView is null,please set MenuView");
		}
	}

	public View getMenuView(){
		if(mMenuViewSmall != null){
			return mMenuViewSmall;
		}else{
...
//		paramsBig.setMargins(widthOffset,heightOffset-actionBarHeight, menuWidth, menuHeight);
		...
		paramsSmall.setMargins(location[0], location[1]-actionBarHeight,
					location[0]+viewWidth, location[1]-actionBarHeight+viewHeight);
